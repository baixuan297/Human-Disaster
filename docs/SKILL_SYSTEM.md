...
**技能 / SkillManager / SkillLookup** 的主文档。
...
| 项 | 说明 |
|----|------|
| **数据来源** | `/game-data/skills`（`GameDataManager` + `SkillResourceRegistry`） |
...
- API 返回的 **`metadata`**（含 **`res_path`、`implemented`**）由 `GameDataManager` 解析；`SkillResourceRegistry` 据此加载 `SkillResource`。
...
角色存档中的技能字段见 [CharacterDataManager.md](CharacterDataManager.md)；REST 路径见 [APIManager.md](APIManager.md)。
