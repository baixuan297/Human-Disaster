...
func _setup_components() -> void:
	if not movement_component:
		return
	movement_component.setup(self, input_controller, raycast3d, de_pie, cucilla, head, animation_tree, player_mesh)
	camera_controller.setup(self, nek, head, eyes, player_camera, third_person, t_person, 10.0)
	interaction_component.setup(interactable_ray, pickray, holdposition, joint, weapon_manager, self)
	player_ui_controller.setup(health_bar, health_label, crosshair, crosshairhit, Ammo_container, no_more_bullet, hit_rect)
